...
export async function getAllPosts ({ includePages = false }) {
  ...
  try {
    ...
    if (!response?.collection || !response?.block) {
      console.error(
    ...
    const pageBlock = response.block?.[id]?.value?.value
    const collectionId = pageBlock?.collection_id || Object.keys(response.collection)[0]
    const collection = response.collection?.[collectionId]?.value?.value
    ...
    for (const viewId of viewIds) {
      const collectionViewRaw = response.collection_view?.[viewId]?.value
      const collectionView = collectionViewRaw?.value || collectionViewRaw
    ...
    for (const [key, val] of Object.entries(rawBlock)) {
      block[key] = { value: val.value?.value ?? {} }
    }
